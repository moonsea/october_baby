...
if($do == 'consume') {
	...
	if(checksubmit()) {
		...
		if (empty($recharges_set['params']['recharge_type'])) {
			...
			$grant_rate = $setting['grant_rate'];
			if($grant_rate > 0) {
				$credit1 = $discount_credit * $grant_rate;
				$log_credit1 = array(
					$_W['member']['uid'],
					"使用会员卡消费【{$discount_credit}】元,消费返积分比率：【1:{$grant_rate}】,共赠送积分{$credit1}"
				);
				mc_credit_update($_W['member']['uid'], 'credit1', $credit1, $log_credit1);
				$discount_str .= "，消费返积分比率：【1:{$grant_rate}】,共赠送积分{$credit1}";
			}
		}
		$log_credit2 = array(
			$_W['member']['uid'],
